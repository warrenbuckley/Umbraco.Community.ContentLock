...
:::note
The **Lock** action is only shown when the node is not already locked. The **Unlock** action appears in its place if the node is locked by you, or if the node is locked by someone else and you have the [Unlocker permission](/permissions/index/).
:::
